...
int main( int argc, char *argv[] )
{
	try
	{
		...
		auto loadedBundle = testEngine.loadBundle( "Bundle_Identification" );
	...
	// -------print the block metainfo---------
		...
		_2Real::app::BlockHandle identificationProvider = loadedBundle.first.createBlock("IdentificationProvider", threadpool, std::vector< _2Real::app::BlockHandle >());

		_2Real::app::BlockIo identificationProviderIO = identificationProvider.getBlockIo();
		auto incInlet = std::dynamic_pointer_cast< _2Real::app::InletHandle >(identificationProviderIO.mInlets[0]);

		std::future< _2Real::BlockResult > setup = identificationProvider.setup();
		handleFuture( setup );
		std::future< _2Real::BlockResult > start = identificationProvider.startUpdating(timer);
		handleFuture( start );
		...
		std::future< _2Real::BlockResult > stop = identificationProvider.stopUpdating();
		handleFuture( stop );
		std::future< _2Real::BlockResult > shutdown = identificationProvider.shutdown();
